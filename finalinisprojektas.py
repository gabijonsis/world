...
def home():
    if 'loggedin' in session:
        ...
        cursor.execute('SELECT * FROM usersdata WHERE id = %s', [session['id']])
...
def login():
    ...
    if request.method == 'POST' and 'lusername' in request.form and 'lpassword' in request.form:
        ...
        cursor.execute('SELECT * FROM usersdata WHERE username = %s', (lusername,))
        ...
        cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cursor.execute("CREATE TABLE IF NOT EXISTS usersdata(id serial PRIMARY KEY, username VARCHAR ( 100 ) NOT NULL, password VARCHAR ( 100 ) NOT NULL, email VARCHAR ( 100 ) NOT NULL, gdata VARCHAR ( 20 ) NOT NULL, country VARCHAR ( 50 ) NOT NULL)")
        ...
        if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form and 'bdate' in request.form and 'country' in request.form:
            ...
            if account:
                ...
                cursor.execute("INSERT INTO usersdata (username, password , email, gdata, country) VALUES (%s,%s,%s,%s,%s)", (username, hashed, email, birthdate, country))
                conn.commit()
    ...
    if request.method == 'POST' and 'timezone' in request.form:
        ...
        for city in allzones:
            if city == timezone:
                ...
                city_time=datetime.datetime.now(city_name)
